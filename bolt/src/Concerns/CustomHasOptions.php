...
use LaraExperts\Bolt\DataSources\FormFileTypes;
...
trait CustomHasOptions
{
    ...
    public static function uploadFileType(): Grid
    {
        return Grid::make()
            ->schema([
                Select::make('options.UploadfileType')
                    ->label(__('Upload File Type'))
                    ->options(
                        FormFileTypes::getOptions()
                ),
            ])
            ->columns(1);
    }
