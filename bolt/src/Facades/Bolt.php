...
use LaraExperts\Bolt\Contracts\CustomFormSchema;
use LaraExperts\Bolt\Contracts\CustomSchema;
...
class Bolt extends Facade
{
    ...
    public static function availableFields(): Collection
    {
        ...
        return Cache::remember('bolt.fields', Carbon::parse('1 month'), function () {
            $coreFields = Collectors::collectClasses(__DIR__ . '/../Fields/Classes', 'LaraExperts\\Bolt\\Fields\\Classes\\');
            $appFields = Collectors::collectClasses(base_path(config('form-bolt.collectors.fields.path')), config('form-bolt.collectors.fields.namespace'));
    ...
    public static function availableDataSource(): Collection
    {
        ...
        return Cache::remember('bolt.dataSources', Carbon::parse('1 month'), function () {
            return Collectors::collectClasses(
                base_path(config('form-bolt.collectors.dataSources.path')),
                config('form-bolt.collectors.dataSources.namespace')
            )
                ->sortBy('sort');
        });
    }
